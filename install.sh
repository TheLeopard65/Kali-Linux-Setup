...
info "[##] Installing Compulsory/Required Tools --------------- [ TOTAL = 29 ]"
...
info "[##] Installing Service-Specific Tools ------------------ [ TOTAL = 21 ]"
...
apt-get -qq install -y sqlmap onesixtyone nbtscan snmp snmpcheck samba-common-bin rpcbind kubectl
...
info "[##] Installing Windows-Specific Tools ------------------- [ TOTAL = 6 ]"
apt-get -qq install -y windows-binaries mimikatz rubeus nishang powersploit laudanum

info "[##] Installing Password Cracking Tools ------------------ [ TOOLS = 6 ]"
apt-get -qq install -y hashid john hashcat hydra medusa cewl cupp
...
if [[ "$pipt" == "y" ]]; then
	info "[##] Installing Important PIPX Tools -------------------- [ TOOLS = 22 ]"
	...
	pipx install --quiet defaultcreds-cheat-sheet kerbrute pywhisker droopescan uploadserver wsgidav cheroot xsstrike wesng > /dev/null
